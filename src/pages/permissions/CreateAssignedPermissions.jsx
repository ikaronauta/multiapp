// src/pages/businesses/CreateAssignedPermissions.jsx
...
import { assignPermission } from "../../adapters/assignedPermissions.adapter";


export default function CreateAssignedPermissions() {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setShowAlert(true);
        setTitleAlert("Error al asignar el permiso");
        setMessageAlert1(errorMsg);
        setMessageAlert2(response?.error?.details ?? "");
        console.error("Error assing permission:", errorMsg);
        return;
      }

      setShowAlertSubmit(false);
      setShowAlert(true);
      setTitleAlert("Permiso asignado");
      setIconComponentModalAlert(<Info className="text-green-600" size={24} />);
      setMessageAlert1("El permiso ha sido asignado correctamente");
  ...
  return (
    <div className="sm:max-w-3xl mx-auto">
      ...
      <div className="relative w-full sm:max-w-3xl mx-auto bg-white shadow-md rounded-lg p-4 sm:p-6">
        <h2 className="text-gray-900 text-2xl font-bold mb-4">Asignar Permiso</h2>
